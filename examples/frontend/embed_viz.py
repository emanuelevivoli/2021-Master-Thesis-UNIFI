...
embedding_type = st.sidebar.selectbox(
    "Select Embeddings", options, format_func=lambda x: embeddings[x])
st.sidebar.text('OR')
uploaded_file = st.sidebar.file_uploader(
    "Upload a file (Optional)", type="txt")
...
def render_plot(fig):
    fig.update_layout(margin={"r": 50, "t": 100, "l": 0,
                      "b": 0}, height=750, width=850)
    st.plotly_chart(fig)
